...
# Additional/Alternative applications can be specifed at runtime. 
BLOCKING_APPS = ['Keynote', 'Microsoft PowerPoint']

# What application is being installed? 
# This will be used to check if the app being updated isn't running, just go ahead and update
# Defaults to Finder so that prompt will be displayed if nothing is specified in policy
UPDATED_APP = ['Finder']
...
# the GUI prompt. We set 0 (i.e. Start Now) as the first and
# therefore default option.
GUI_DEFER_OPTIONS = ["0", "60", "300", "1800", "3600", "14400", "43200"]
GUI_BUTTON = "Okay"

# Confirmation dialog Config
GUI_S_HEADING = "Update scheduled"
GUI_S_ICON = ("/Library/User Pictures/McAfee.png")
GUI_S_BUTTON = "OK"
...
def detect_prompt_justification():
    """Checks if the app being updated is running.
   
    Returns:
        (bool) true/false if any updated app is running
    """
    prompt_needed = False
    print UPDATED_APP
    running_apps = get_running_apps()
    for app in UPDATED_APP:
        if app in running_apps:
            print "App {} is running, prompt needed".format(app)
            prompt_needed = True
...
def main():
    ...
    # If nothing was specified, use the defaults specifed at top. Otherwise:
    if args.blockers != "":
        # Use what was passed
        BLOCKING_APPS = args.blockers.split(",")
        
    # If nothing was specified, use the default (Finder) specifed at top. Otherwise:
    ...
    if args.mode == 'prompt':
        ...
        # Check if prompt is needed, otherwise just do the needful 
        if detect_prompt_justification(): 
